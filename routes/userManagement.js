...
const { authLoggerObj, userLoggerObj } = require('../utils/LoggerConstants');
...
router.post('/create', async (req, res) => {
  try {
    ...
    auditlogsControllers.setAuditLogs(req, {
      action_type: 'create',
      module_name: 'User Management',
      email: req.user ? req.user.email : 'Unknown',
      remark: `User created a new user: ${newUser[0].full_name}`,
    });
...
router.put('/:user_id', async (req, res) => {
  try {
    ...
    // Log to audit trail
    console.log('Logging update to audit trail', req.user);
    await auditlogsControllers.setAuditLogs(req, {
      action_type: 'update',
      module_name: 'User Management',
      oldValue: oldUser[0],
      newValue: updatedUser[0],
      email: req.user ? req.user.email : 'Unknown',
      remark: `User updated a user: ${updatedUser[0].full_name}`,
    });
...
router.delete('/:user_id', async (req, res) => {
  try {
    ...
    await auditlogsControllers.setAuditLogs(req, {
      ...
      email: req.user ? req.user.email : 'Unknown',
      remark: `User deleted a user: ${user[0].full_name}`,
    });
    // Hard delete - actually remove the record
    await db.query('DELETE FROM sp_user_master WHERE user_id = ?', [user_id]);
